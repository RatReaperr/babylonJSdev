...
function createDirectionalShadows(light: DirectionalLight, sphere: Mesh ,box: Mesh){
    ...
    function getMaterial(scene: Scene){
      ...
      myMaterial.emissiveColor = new Color3(1, .4, 0.5);
   ...
   function createSphere(scene: Scene) {
      ...
      "ellipsoid",
      { diameter: .7, diameterY : 2, segments: 16 },
      scene,
    );
    sphere.position.x = 0;
    sphere.position.y = 1;
  ...
  function createCylinder(scene : Scene) {
    ...
    const cone = MeshBuilder.CreateCylinder("Cone", {diameterTop:0, diameterBottom:1, height:3}, scene)
    cone.position.x = 7;
    cone.position.y = 1;
    return cone;
  }
   ...
   function createCapsule(scene : Scene) {
    const capsule = MeshBuilder.CreateCapsule("triangle", {radius:0.5, height:2, tessellation: 4, subdivisions: 4}, scene)
